...
import {
  PlusOutlined, CheckOutlined, CloseOutlined, ToolOutlined, AuditOutlined, GiftOutlined,
...
import {
  MR_STATUS_META, EQUIPMENT_TYPE_META, PRIORITY_META, OFFICE_LOCATIONS, GENERATOR_LIST,
} from '../../../types';
...
function buildColumns(onView: (r: EquipmentMaintenance) => void): ColumnsType<EquipmentMaintenance> {
  return [
    ...
    { title: 'Type', dataIndex: 'maintenanceType', key: 'type', width: 155,
    ...
    { title: 'Fault ID', dataIndex: 'faultIdentified', key: 'fault', width: 180, ellipsis: true,
      render: (v?: string) => v
    ...
    { title: '', key: 'action', width: 65,
      render: (_: unknown, r: EquipmentMaintenance) =>
...
export default function EquipmentTab() {
  ...
  const act = async (fn: () => Promise<unknown>) => {
    ...
    try {
      await fn(); refresh();
      if (selected) equipmentMaintenanceApi.getById(selected.id).then(setSelected).catch(() => {});
    } catch (e: unknown) {
  ...
  const handleCreate = async (values: Record<string, unknown>) => {
    ...
    try {
      ...
      const isGen = GENERATOR_TYPES.includes(values.maintenanceType as string);
      const gen = isGen ? GENERATOR_LIST.find(g => g.assetNo === values.generatorSelect) : null;
      await equipmentMaintenanceApi.create({
        assetNo:          isGen && gen ? gen.assetNo : (values.assetNo as string).trim(),
        assetDescription: isGen && gen ? gen.description : (values.assetDescription as string).trim(),
        maintenanceType:  values.maintenanceType as string,
        endUser:          (values.endUser as string).trim(),
        location:         isGen && gen ? gen.location : location,
  ...
  const handleAssess = async (values: Record<string, unknown>) => {
    if (!selected) return;
    await act(() => equipmentMaintenanceApi.assess(selected.id, {
  ...
  const handleComplete = async (values: Record<string, unknown>) => {
    if (!selected) return;
    await act(() => equipmentMaintenanceApi.complete(selected.id, {
  ...
  const handleHandover = async (values: Record<string, unknown>) => {
    if (!selected) return;
    await act(() => equipmentMaintenanceApi.handover(selected.id, {
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 12 }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          <Input.Search placeholder="Search asset, type, end user…" style={{ width: 280 }} allowClear
      ...
      <Modal title={<><ToolOutlined /> New Equipment Maintenance Request</>}
        ...
        onCancel={() => { setCreateOpen(false); createForm.resetFields(); setLocationSel(null); setSelectedGen(null); setSelectedType(null); setCreateError(null); }}
            ...
            <Col span={12}>
              <Form.Item name="maintenanceType" label="Maintenance Type" rules={[{ required: true }]}>
                <Select placeholder="Select type…"
                  onChange={(v: string) => { setSelectedType(v); setSelectedGen(null); createForm.setFieldsValue({ generatorSelect: undefined, assetNo: '', assetDescription: '' }); }}
                  options={EQ_TYPES.map(t => ({ value: t, label: EQUIPMENT_TYPE_META[t as keyof typeof EQUIPMENT_TYPE_META]?.label ?? t }))} />
              </Form.Item>
            </Col>
            <Col span={12}>
      ...
      <Modal title={<><AuditOutlined /> Record Fault Assessment</>} open={assessOpen}
        ...
        <Form form={assessForm} layout="vertical" onFinish={handleAssess} initialValues={{ partsRequired: false }}>
          ...
          <Form.Item name="partsRequired" label="Parts / Spares Required?" valuePropName="checked">
            <Switch checkedChildren="Yes" unCheckedChildren="No"
              onChange={(v: boolean) => { setPartsRequired(v); if (!v) setPartsSource(null); }} />
          </Form.Item>
          {partsRequired && (
            <>
              <Form.Item name="partsSource" label="Source of Parts">
          ...
          <Form.Item name="sparesCostNaira" label="Estimated Spares Cost (₦)">
      ...
      <Modal title={<><CheckOutlined /> Mark as Completed</>} open={completeOpen}
        ...
        <Form form={completeForm} layout="vertical" onFinish={handleComplete}>
          <Form.Item name="workDone" label="Description of Work Done" rules={[{ required: true }]}>
            <TextArea rows={4} placeholder="Describe work performed, parts replaced…" maxLength={2000} showCount />
          </Form.Item>
      ...
      {/* Handover modal */}
      <Modal title={<><GiftOutlined /> Confirm Equipment Handover</>} open={handoverOpen}
        ...
        {actionError && <Alert message={actionError} type="error" showIcon style={{ marginBottom: 12 }} />}
        <p>Confirm that <strong>{selected?.assetDescription}</strong> has been returned to the end user.</p>
        ...
        <Drawer
          title={
            <Space>
              ...
              <span style={{ fontWeight: 600 }}>{selected.assetDescription}</span>
              <Text type="secondary" style={{ fontSize: 12 }}>{selected.assetNo}</Text>
            </Space>
          }
            ...
            <Space wrap>
              ...
              {isApprover && selected.status === 'Approved' && (
                <Button type="primary" size="small"
                  onClick={() => act(() => equipmentMaintenanceApi.updateStatus(selected.id, 'Ongoing'))}>Mark Ongoing</Button>
              )}
        ...
        >
          ...
          <Descriptions column={1} size="small" bordered>
            ...
            {selected.nextServiceHour != null && (
              <Descriptions.Item label="Next Service At">{selected.nextServiceHour.toLocaleString()} h</Descriptions.Item>
            )}
          ...
          {selected.partsRequired && (
            <>
              <Divider titlePlacement="left" orientationMargin={0} style={{ fontSize: 12 }}>Parts &amp; Procurement</Divider>
              <Descriptions column={1} size="small" bordered>
                <Descriptions.Item label="Parts Required"><Tag color="orange">Yes</Tag></Descriptions.Item>
                ...
                {selected.sparesCostNaira != null && (
                  <Descriptions.Item label="Spares Cost">
          ...
          {selected.handoverConfirmed && (
            <>
              ...
              <Descriptions column={1} size="small" bordered>
                <Descriptions.Item label="Status"><Tag color="green" icon={<CheckOutlined />}>Equipment Returned</Tag></Descriptions.Item>
      ...
      <Modal title="Reject Request" open={rejectOpen}
        onOk={async () => {
          ...
          await act(() => equipmentMaintenanceApi.reject(selected.id, rejectReason));
